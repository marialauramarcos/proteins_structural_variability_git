...
# The program only mutates CMs, but considers both CAs and CMs.
...
GenerateMutantsCM <- function(family,
                              chain.p.ref,
                              n.mut.p,
                              fmax, 
                              R0,
  ...
  m.r.mut = matrix(0, nrow = 3 * n.sites.tot, ncol = n.prot * n.mut.p)
  ...
  for (P in (1:n.prot)) {
    ...
    for(mut in seq(1:n.mut.p)) {
      ...
      sites.to.mutate = seq((n.sites + 1), n.sites.tot) 
  ...
  write.csv(as.vector(r.ca.p.ref), file = file.path(out.dir, paste(mut.fname.id, "_out_r.p.ref.csv", sep = "")), row.names = FALSE)

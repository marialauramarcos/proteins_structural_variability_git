ReadFasta <- function (file, rm.dup = TRUE, to.upper = FALSE, to.dash = TRUE) 
{
  ...
  if (to.dash) {
    raw.fa[-ind] <- gsub("[/.]", "-", raw.fa[-ind])
  }
  if (to.upper) {
    raw.fa[-ind] <- toupper(raw.fa[-ind])
  }
  ind.s <- ind + 1
  ...
  seq.format <- function(x, max.seq = max(seq.dim)) {
    fa <- rep("-", max.seq)
    fa[c(1:x[3])] <- unlist(strsplit(raw.fa[(x[1]:x[2])], 
                                     split = ""))
    return(fa)
  }
  ...
  if (rm.dup) {
    ...
    if (any(dups)) {
      print(paste(" ** Duplicated sequence id's: ", row.names(store.fa)[dups], 
                  " **", sep = ""))

...
if (data.dir == "OUT/out_subset_CM_ANM") { 
  ...
  r.p.ref = c(ref.pdb$xyz[inds$xyz], as.vector(r.CM.p.ref)) 
...
### Sequence (1/CN) - structure (SDi) comparison
In this section we compare theoretical and experimental structural divergence profiles with CN.  
...
plot(x = d.exp$mean.SD, y = (1/CN), xlab = "<SDi> experimental")
plot(x = d.mut$mean.SD, y = (1/CN), xlab = "<SDi> no-selection")
plot(x = d.medium$mean.SD, y = (1/CN), xlab = "<SDi> selection")

## calculate Person correlations
r.exp.mean.SD.CN = cor(d.exp$mean.SD, (1/CN))
r.mut.mean.SD.CN = cor(d.mut$mean.SD, (1/CN))
r.medium.mean.SD.CN = cor(d.medium$mean.SD, (1/CN))

cor.df = data.frame("comparison" = c("SDi experimental vs 1/CN", "SDi no-selection vs 1/CN", "SDi selection vs 1/CN"),
                    "R" = c(r.exp.mean.SD.CN , r.mut.mean.SD.CN, r.medium.mean.SD.CN))

knitr::kable(cor.df, digits = 2, caption = "Pearson Correlation structure (SDi) - sequence (1/CN)")

## calculate Spearman correlations
r.exp.mean.SD.CN = cor(d.exp$mean.SD, (1/CN), method = "spearman")
r.mut.mean.SD.CN = cor(d.mut$mean.SD, (1/CN), method = "spearman")
r.medium.mean.SD.CN = cor(d.medium$mean.SD, (1/CN), method = "spearman")

cor.df = data.frame("comparison" = c("SDi experimental vs 1/CN", "SDi no-selection vs 1/CN", "SDi selection vs 1/CN"),
                    "R" = c(r.exp.mean.SD.CN , r.mut.mean.SD.CN, r.medium.mean.SD.CN))

knitr::kable(cor.df, digits = 2, caption = "Spearman Correlation structure (SDi) - sequence (1/CN)")
```
